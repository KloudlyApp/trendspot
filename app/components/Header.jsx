      ...
      >
        ...
        <ul className='mt-20 gap-6 flex flex-col'>
          {navigationLinks.map(({ path, name, icon }) => (
            <li
              className='capitalize hover:text-blue-400 hover:transition-all hover:delay-150'
              key={name}
            >
              <Link href={path}>{toggleSideBar ? name : icon}</Link>
            </li>
          ))}
        </ul>

        <div className='flex absolute bottom-16 gap-2 items-center'>
